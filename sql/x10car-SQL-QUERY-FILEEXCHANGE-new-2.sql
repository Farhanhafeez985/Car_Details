...
-- LANGUAGE
SET @lang = 'de';
SET @SPRACHNR = IFNULL((SELECT SPRACHNR FROM `020` WHERE ISO_CODE = @lang LIMIT 1), 1);  -- TECDOC ID LANG regarding iso_code language, DEFAULT 4 = EN
...
SELECT
		...
		IF(@NEEDSHOW = 'Compatibility' AND T400.VKNZIELART = 2,
			CONCAT('Ktype=',T400.VKNZIELNR,
					IFNULL(CONCAT('|Notes=',GET_LIST_TERMSOFUSE_TAF(T.DLNR, T.ARTNR, T400.VKNZIELNR, @lang)), '')
				),
			''
